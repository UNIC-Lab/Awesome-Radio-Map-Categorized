# Awesome Radio Map

> A curated collection of learning-based radio maps, radio environment maps (REM), channel maps, channel knowledge maps (CKM), and wireless radiance fields based on NeRF or 3D Gaussian splatting.



## Citation

If you find this repository helpful, please cite the tutorial paper:

> [A Tutorial on Learning-Based Radio Map Construction: Data, Paradigms, and Physics-Awareness](https://arxiv.org/abs/2603.17499)



```bibtex
@misc{wang2026tutoriallearningbasedradiomap,
  title        = {A Tutorial on Learning-Based Radio Map Construction: Data, Paradigms, and Physics-Awareness},
  author       = {Xiucheng Wang and Yuhao Pan and Nan Cheng and Çağkan Yapar and Ruijin Sun and Zhisheng Yin and Conghao Zhou and Wenchao Xu and Yuxiang Zhang and Jianhua Zhang and Shuguang Cui and Xuemin Shen},
  year         = {2026},
  eprint       = {2603.17499},
  archivePrefix= {arXiv},
  primaryClass = {eess.SY},
  url          = {https://arxiv.org/abs/2603.17499}
}
```

</details>

## Browse

| Collection | Journals | Conferences | Preprints | Total |
|:--|--:|--:|--:|--:|
| [With code](#papers-with-code) | 20 | 7 | 15 | **42** |
| [Without code](#papers-without-code) | 142 | 149 | 49 | **340** |
| **All papers** | **162** | **156** | **64** | **382** |

---

## Papers with Code

### Journals

<details open>
<summary><strong>2026</strong> <sub>9 papers</sub></summary>

- **A Fine-Grained 3D Radio Map Construction Paradigm With Ultra-Low Sampling Rates by Large Generative Models**  
  *IEEE JSAC 2026* · [Paper](https://ieeexplore.ieee.org/abstract/document/11455177) · [Code](https://github.com/liuzhiyuan-pku/RadioLAM) · <sub>JCR Q1 · CAS 1 · IF 16.8 · CCF A</sub>
- **Bridging Visual and Wireless Sensing via a Unified Radiation Field for 3D Radio Map Construction**  
  *npj Wireless Technology 2026* · [Paper](https://doi.org/10.1038/s44459-026-00067-6) · [Code](https://github.com/wenchaozheng/URF-GS) · <sub>JCR None · JCR None · IF None · CCF None</sub>
- **PathFinder: Advancing Path Loss Prediction for Single-to-Multi-Transmitter Scenario**  
  *Pattern Recognition 2026* · [Paper](https://arxiv.org/abs/2512.14150) · [Code](https://github.com/EmorZz1G/PathFinder) · <sub>JCR Q1 · CAS 1 · IF 9.1 · CCF B</sub>
- **R2Net: 2D Deep Residual Learning with Height Embedding for 3D Radio Map Estimation**  
  *IEEE TVT 2026* · [Paper](https://ieeexplore.ieee.org/abstract/document/11505057) · [Code](https://github.com/lighttime2023/3DiRM3200.git) · <sub>JCR Q1 · CAS 2 · IF 7.5 · CCF None</sub>
- **RadioDiff-$k^2$: Helmholtz Equation Informed Generative Diffusion Model for Multi-Path Aware Radio Map Construction**  
  *IEEE JSAC 2026* · [Paper](https://ieeexplore.ieee.org/document/11278649) · [Code](https://github.com/UNIC-Lab/RadioDiff-k) · <sub>JCR Q1 · CAS 1 · IF 16.8 · CCF A</sub>
- **RadioDiff-FS: Physics-Informed Manifold Alignment in Few-Shot Diffusion Models for High-Fidelity Radio Map Construction**  
  *IEEE IoT J 2026* · [Paper](https://ieeexplore.ieee.org/document/11577136) · [Code](https://github.com/UNIC-Lab/RadioDiff-FS) · <sub>JCR Q1 · CAS 1 · IF 8.7 · CCF C</sub>
- **RadioDiff-Inverse: Diffusion Enhanced Bayesian Inverse Estimation for ISAC Radio Map Construction**  
  *IEEE TWC 2026* · [Paper](https://ieeexplore.ieee.org/document/11466421) · [Code](https://github.com/UNIC-Lab/radiodiff-inverse) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **RadioMapMotion: A Dataset and Benchmark for Proactive Spatio-Temporal Radio Environment Prediction**  
  *IEEE TCCN 2026* · [Paper](https://ieeexplore.ieee.org/abstract/document/11488322) · [Code](https://github.com/UNIC-Lab/RadioMapMotion) · <sub>JCR Q1 · CAS 2 · IF 8.0 · CCF None</sub>
- **RadioTrace: Transmitter-Aware Diffusion for Radio Map Estimation Without Deployment-Time Fine-Tuning**  
  *IEEE TWC 2026* · [Paper](https://doi.org/10.1109/TWC.2026.3716965) · [Code](https://github.com/YannLeo/RadioTrace) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>

</details>

<details open>
<summary><strong>2025</strong> <sub>5 papers</sub></summary>

- **Neural Representation for Wireless Radiation Field Reconstruction: A 3D Gaussian Splatting Approach**  
  *IEEE TWC 2025* · [Paper](https://doi.org/10.1109/TWC.2025.3631663) · [Code](https://github.com/wenchaozheng/WRF-GSplus) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **Paying Deformable Attention to Sparse Spatial Observations for Deep Radio Map Estimation**  
  *IEEE TCCN 2025* · [Paper](https://ieeexplore.ieee.org/document/11176936) · [Code](https://github.com/AkonLau/DAT-Unet) · <sub>JCR Q1 · CAS 2 · IF 8.0 · CCF None</sub>
- **RadioDiff-3D: A 3D × 3D Radio Map Dataset and Generative Diffusion-Based Benchmark for 6G Environment-Aware Communication**  
  *IEEE TNSE 2025* · [Paper](https://ieeexplore.ieee.org/document/11083758) · [Code](https://github.com/UNIC-Lab/UrbanRadio3D) · <sub>JCR Q1 · CAS 2 · IF 7.3 · CCF None</sub>
- **RadioDiff: An Effective Generative Diffusion Model for Sampling-Free Dynamic Radio Map Construction**  
  *IEEE TCCN 2025* · [Paper](https://ieeexplore.ieee.org/document/10764739) · [Code](https://github.com/UNIC-Lab/RadioDiff) · <sub>JCR Q1 · CAS 2 · IF 8.0 · CCF None</sub>
- **RadioMamba: Breaking the Accuracy-Efficiency Trade-Off in Radio Map Construction Via a Hybrid Mamba-UNet**  
  *IEEE TNSE 2025* · [Paper](https://ieeexplore.ieee.org/document/11190042) · [Code](https://github.com/UNIC-Lab/RadioMamba) · <sub>JCR Q1 · CAS 2 · IF 7.3 · CCF None</sub>

</details>

<details open>
<summary><strong>2024 and Earlier</strong> <sub>6 papers</sub></summary>

- **ACT-GAN: Radio map construction based on generative adversarial networks with ACT blocks**  
  *IET Communications 2024* · [Paper](https://ietresearch.onlinelibrary.wiley.com/doi/full/10.1049/cmu2.12846) · [Code](https://github.com/YNUniversityCQ/ACT-GAN) · <sub>JCR None · CAS None · IF None · CCF C</sub>
- **Deep Completion Autoencoders for Radio Map Estimation**  
  *IEEE TWC 2022* · [Paper](https://ieeexplore.ieee.org/document/9523765) · [Code](https://github.com/fachu000/deep-autoencoders-cartography) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **RadioGAT: A Joint Model-Based and Data-Driven Framework for Multi-Band Radiomap Reconstruction via Graph Attention Networks**  
  *IEEE TWC 2024* · [Paper](https://ieeexplore.ieee.org/document/10682510) · [Code](https://github.com/Lxj321/RadioGAT-Multi-band-Radiomap-Reconstruction) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **RadioUNet: Fast Radio Map Estimation With Convolutional Neural Networks**  
  *IEEE TWC 2021* · [Paper](https://ieeexplore.ieee.org/document/9354041) · [Code](https://github.com/RonLevie/RadioUNet) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **Real-Time Outdoor Localization Using Radio Maps: A Deep Learning Approach**  
  *IEEE TWC 2023* · [Paper](https://ieeexplore.ieee.org/document/10122907) · [Code](https://github.com/CagkanYapar/LocUNet) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **RME-GAN: A Learning Framework for Radio Map Estimation Based on Conditional Generative Adversarial Network**  
  *IEEE IoT J 2023* · [Paper](https://ieeexplore.ieee.org/document/10130091) · [Code](https://github.com/achinthaw/RME-GAN) · <sub>JCR Q1 · CAS 1 · IF 8.7 · CCF C</sub>

</details>

### Conferences

<details open>
<summary><strong>2026</strong> <sub>3 papers</sub></summary>

- **iRadioDiff: Physics Informed Diffusion Model for Effective Indoor Radio Map Construction and Localization**  
  *IEEE ICC 2026* · [Paper](https://arxiv.org/abs/2511.20015) · [Code](https://github.com/UNIC-Lab/iRadioDiff) · <sub>CCF C</sub>
- **OpenPathNet: An Open-Source RF Multipath Data Generator for AI-Driven Wireless Systems**  
  *IEEE VTC2026-Spring* · [Paper](https://arxiv.org/abs/2512.17286) · [Code](https://github.com/liu-lz/OpenPathNet) · <sub>CCF None</sub>
- **Physics-Informed VAE-EVT for Tail-Aware Radio Map Prediction**  
  *IEEE GLOBECOM 2026* · [Paper](https://arxiv.org/abs/2608.15314) · [Code](https://github.com/AmandaGamage/physics-informed-vae-evt) · <sub>CCF C</sub>

</details>

<details open>
<summary><strong>2025</strong> <sub>3 papers</sub></summary>

- **PhyRMDM: Physics-Informed Representation Alignment for Sparse Radio-Map Reconstruction**  
  *ACM MM BNI 2025* · [Paper](https://doi.org/10.1145/3746027.3758161) · [Code](https://github.com/Hxxxz0/RMDM) · <sub>CCF None</sub>
- **SIP2Net: Situational-Aware Indoor Pathloss-Map Prediction Network for Radio Map Generation**  
  *IEEE ICASSP 2025* · [Paper](https://ieeexplore.ieee.org/document/10890319) · [Code](https://github.com/lululu123321/SIP2Net) · <sub>CCF B</sub>
- **TransPathNet: A Novel Two-Stage Framework for Indoor Radio Map Prediction**  
  *IEEE ICASSP 2025* · [Paper](https://ieeexplore.ieee.org/document/10887939) · [Code](https://github.com/LiXin97/TransPathNet) · <sub>CCF B</sub>

</details>

<details open>
<summary><strong>2024 and Earlier</strong> <sub>1 paper</sub></summary>

- **PMNet: Robust Pathloss Map Prediction via Supervised Learning**  
  *IEEE GlobeCom 2023* · [Paper](https://ieeexplore.ieee.org/document/10437562) · [Code](https://github.com/abman23/PMNet) · <sub>CCF C</sub>

</details>

### Preprints

<details open>
<summary><strong>2026</strong> <sub>10 papers</sub></summary>

- **ControlRadio: Prompt-Driven Controllable Diffusion for Cross-Modal Radio Map Generation**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2608.09357) · [Code](https://github.com/AkonLau/ControlRadio)
- **DF-3DRME: A Data-Friendly Learning Framework for 3D Radio Map Estimation based on Super-Resolution Technique**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2604.00676) · [Code](https://github.com/LINZHU-PolyU/DF-3DRME)
- **PILOT: One Physics-Integrated Generation Framework to Unify 2D and 3D Radio Map Construction**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2604.23533) · [Code](https://github.com/Uminan/PILOT)
- **Point2Radio: A Foundation Model for Cross-Scene Radio Fields from Material-Aware Point Clouds**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2607.28994) · [Code](https://github.com/wenchaozheng/Point2Radio)
- **RadioDiff-Inv2: Differentiable Diffusion Inversion under Location Drift from Sparse Noisy Measurements for Radio Map Estimation**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2606.08439) · [Code](https://github.com/UNIC-Lab/RadioDiff-inv2)
- **RadioDiff-v2: Generative Angular Radio Maps for Multi-Beam Selection and Localization**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2607.08045) · [Code](https://github.com/UNIC-Lab/RadioDiff-v2)
- **RadioFormer3D: Weakly Supervised 3D Radio Map Estimation in Low-Altitude Airspace via Generative Modeling**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2605.29538) · [Code](https://github.com/FzJun26th/RadioFormer3D)
- **U6G XL-MIMO Radiomap Prediction: Multi-Config Dataset and Beam Map Approach**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2603.06401v1) · [Code](https://github.com/Lxj321/MulticonfigRadiomapDataset)
- **XFreq-GS: Cross-Frequency Wireless Radiation Field Reconstruction with 3D Gaussian Splatting**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2605.11432) · [Code](https://github.com/KINGAZ1019/XFreq-GS)
- **Zero-Shot Frequency Generalization for Radio Map Prediction via Cross-Attention Physics-Residual Learning**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2608.16194) · [Code](https://github.com/sajjadhussa1n/PRCA-Net)

</details>

<details open>
<summary><strong>2025</strong> <sub>4 papers</sub></summary>

- **FERMI: Flexible Radio Mapping with a Hybrid Propagation Model and Scalable Autonomous Data Collection**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2504.14862) · [Code](https://github.com/ymLuo1214/Flexible-Radio-Mapping)
- **GSRF: Complex-Valued 3D Gaussian Splatting for Efficient Radio-Frequency Data Synthesis**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2502.01826) · [Code](https://github.com/nesl/GSRF)
- **RadioFormer: A Multiple-Granularity Radio Map Estimation Transformer with 0.01% Spatial Sampling**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2504.19161) · [Code](https://github.com/FzJun26th/RadioFormer)
- **RF-3DGS: Wireless Channel Modeling with Radio Radiance Field and 3D Gaussian Splatting**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2411.19420) · [Code](https://github.com/SunLab-UGA/RF-3DGS)

</details>

<details open>
<summary><strong>2024 and Earlier</strong> <sub>1 paper</sub></summary>

- **Radio Map Estimation -- An Open Dataset with Directive Transmitter Antennas and Initial Experiments**  
  *arXiv 2024* · [Paper](https://arxiv.org/abs/2402.00878) · [Code](https://github.com/fabja19/RML)

</details>

---

## Papers without Code

### Journals

<details open>
<summary><strong>2026</strong> <sub>48 papers</sub></summary>

- **28 GHz Indoor Continuous-Space Channel Measurements and AI-Enabled 6G Channel Map Construction**  
  *IEEE TCom 2026* · [Paper](https://ieeexplore.ieee.org/abstract/document/11506340) · <sub>JCR Q1 · CAS 2 · IF 8.4 · CCF B</sub>
- **A Low-Complexity Channel Knowledge Map Construction Based on Environmental Partitioning and Interpolation Weight Learning**  
  *IEEE IoT J 2026* · [Paper](https://doi.org/10.1109/JIOT.2026.3672412) · <sub>JCR Q1 · CAS 1 · IF 8.7 · CCF C</sub>
- **A Novel 6G Dynamic Channel Map Based on a Hybrid Channel Model**  
  *IEEE TVT 2026* · [Paper](https://doi.org/10.1109/tvt.2025.3602807) · <sub>JCR Q1 · CAS 2 · IF 7.5 · CCF None</sub>
- **A Three-Stage Vision-Based Transformer for Channel Gain Map Construction**  
  *IEEE TCom 2026* · [Paper](https://ieeexplore.ieee.org/abstract/document/11514075) · <sub>JCR Q1 · CAS 2 · IF 8.4 · CCF B</sub>
- **Active Sampling and Gaussian Reconstruction for Radio-Frequency Radiance Field**  
  *IEEE JSAIT 2026* · [Paper](https://doi.org/10.1109/JSAIT.2026.3688148) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Adaptive Multi-Modal Channel Knowledge Map Construction for Robust Beam Prediction**  
  *IEEE TVT 2026* · [Paper](https://doi.org/10.1109/TVT.2026.3703956) · <sub>JCR Q1 · CAS 2 · IF 7.5 · CCF None</sub>
- **Adversarial Learning-Based Radio Map Reconstruction for Fingerprinting Localization**  
  *IEEE AWPL 2026* · [Paper](https://doi.org/10.1109/lawp.2026.3679273) · <sub>JCR Q2 · CAS 3 · IF 4.6 · CCF None</sub>
- **An RT-Supervised Simulation-to-Simulation Framework for Path Loss Radio Map Prediction Based on Geographic Environmental Information**  
  *Electronics 2026* · [Paper](https://www.mdpi.com/2079-9292/15/8/1750) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Bayesian Neural Field-Based 3-D Radio Environment Map Reconstruction With Uncertainty Quantification Under Sparse Sampling**  
  *IEEE OJ-COMS 2026* · [Paper](https://doi.org/10.1109/OJCOMS.2026.3714834) · <sub>JCR Q1 · CAS 2 · IF 6.1 · CCF None</sub>
- **BeamCKM: A Framework of Channel Knowledge Map Construction for Multi-Antenna Systems**  
  *IEEE TWC 2026* · [Paper](https://doi.org/10.1109/TWC.2026.3718444) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **Blind Radio Map Construction via Topology Guided Manifold Learning**  
  *IEEE IoT J 2026* · [Paper](https://ieeexplore.ieee.org/abstract/document/11474342) · <sub>JCR Q1 · CAS 1 · IF 8.7 · CCF C</sub>
- **Channel Gain Map Estimation Based on 3-D Virtual Scatterer Model**  
  *IEEE TWC 2026* · [Paper](https://doi.org/10.1109/TWC.2026.3685411) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **Channel Knowledge Map Construction via Guided Flow Matching**  
  *IEEE WCL 2026* · [Paper](https://doi.org/10.1109/LWC.2026.3693360) · <sub>JCR Q1 · CAS 2 · IF 5.1 · CCF None</sub>
- **Channel Knowledge Map Construction With Radio Propagation Graph Representation Learning**  
  *IEEE IoT J 2026* · [Paper](https://doi.org/10.1109/JIOT.2026.3672928) · <sub>JCR Q1 · CAS 1 · IF 8.7 · CCF C</sub>
- **Channel Knowledge Map Construction: Recent Advances and Open Challenges**  
  *IEEE Wireless Communications 2026* · [Paper](https://doi.org/10.1109/MWC.2026.3668205) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Constructing Radio Map via Spatio-Temporal Learning of Crowdsourced Trajectory Data**  
  *IEEE TMC 2026* · [Paper](https://doi.org/10.1109/TMC.2026.3710356) · <sub>JCR Q1 · CAS 1 · IF 8.8 · CCF A</sub>
- **CSI-Tuples-Based 3-D Channel Fingerprints Construction Assisted by Multimodal Learning**  
  *IEEE TWC 2026* · [Paper](https://ieeexplore.ieee.org/document/11534430) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **Deformable 2D Gaussian Splatting for Efficient Wireless Radiance Field Rendering**  
  *IEEE TVCG 2026* · [Paper](https://doi.org/10.1109/TVCG.2026.3681115) · <sub>JCR Q1 · CAS 1 · IF 6.8 · CCF A</sub>
- **Digital Twin-Assisted 3D Channel Knowledge Map Inference and Multi-Type Base Station Deployment Optimization for Urban Low-Altitude Economy**  
  *IEEE TMC 2026* · [Paper](https://doi.org/10.1109/tmc.2026.3692307) · <sub>JCR Q1 · CAS 1 · IF 8.8 · CCF A</sub>
- **End-to-End UAV-Enabled Adaptive 3D Radio Mapping via Joint Optimization of Sparse Sampling and Reconstruction**  
  *IEEE IoT J 2026* · [Paper](https://ieeexplore.ieee.org/abstract/document/11456505) · <sub>JCR Q1 · CAS 1 · IF 8.7 · CCF C</sub>
- **F4-CKM: Learning Channel Knowledge Map With Radio Frequency Radiance Field Rendering**  
  *IEEE TCOM 2026* · [Paper](https://doi.org/10.1109/TCOMM.2026.3668162) · <sub>JCR Q1 · CAS 2 · IF 8.4 · CCF B</sub>
- **Fast Aerial Channel Knowledge Map Construction via an Energy-Constrained UAV**  
  *IEEE TVT 2026* · [Paper](https://doi.org/10.1109/TVT.2026.3693060) · <sub>JCR Q1 · CAS 2 · IF 7.5 · CCF None</sub>
- **Flow Matching for Fast Channel Knowledge Map Construction in AI-Native 6G Networks**  
  *IEEE WCL 2026* · [Paper](https://doi.org/10.1109/LWC.2026.3720195) · <sub>JCR Q1 · CAS 2 · IF 5.1 · CCF None</sub>
- **Geometry-Aware Cholesky Projection for Indoor Radio Map Sampling**  
  *IEEE SPL 2026* · [Paper](https://doi.org/10.1109/lsp.2025.3636448) · <sub>JCR None · CAS None · IF None · CCF C</sub>
- **HiFiRadio: High-Fidelity Radio Map Reconstruction for 3D Real-World Scenes**  
  *Technologies 2026* · [Paper](https://doi.org/10.3390/technologies14010058) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Invertible Diffusion for Low-Memory Channel Gain Map Construction in Wireless Communication Networks**  
  *IEEE CL 2026* · [Paper](https://ieeexplore.ieee.org/abstract/document/11481010) · <sub>JCR Q2 · CAS 3 · IF 4.5 · CCF None</sub>
- **Multiple Source Localization via Local Radio Map Construction in Urban Environments**  
  *IEEE TCCN 2026* · [Paper](https://doi.org/10.1109/tccn.2025.3635590) · <sub>JCR Q1 · CAS 2 · IF 8.0 · CCF None</sub>
- **Neural Fitting for Sparse Radio Map Construction in LEO Satellite Network**  
  *IEEE TMC 2026* · [Paper](https://ieeexplore.ieee.org/abstract/document/11481652) · <sub>JCR Q1 · CAS 1 · IF 8.8 · CCF A</sub>
- **Physics-Guided Neural Radiance Field Network for Multi-Antenna Channel Prediction**  
  *IEEE AWPL 2026* · [Paper](https://doi.org/10.1109/LAWP.2026.3683930) · <sub>JCR Q2 · CAS 3 · IF 4.6 · CCF None</sub>
- **PhySNet: Physics-Inspired Spectral-Spatial Decoupled Attention for Radio Map Estimation**  
  *IEEE CL 2026* · [Paper](https://doi.org/10.1109/LCOMM.2026.3705413) · <sub>JCR Q2 · CAS 3 · IF 4.5 · CCF None</sub>
- **Point Cloud Environment-Based Channel Knowledge Map Construction**  
  *IEEE TCCN 2026* · [Paper](https://doi.org/10.1109/TCCN.2026.3706507) · <sub>JCR Q1 · CAS 2 · IF 8.0 · CCF None</sub>
- **Prior Information for 6G Network: From Channel Knowledge Map to Radio Profile**  
  *IEEE Communications Magazine 2026* · [Paper](https://doi.org/10.1109/mcom.001.2600181) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Radio Map Estimation via Latent Domain Plug-and-Play Denoising**  
  *IEEE TSP 2026* · [Paper](https://doi.org/10.1109/TSP.2025.3650699) · <sub>JCR Q1 · CAS 1 · IF 5.5 · CCF None</sub>
- **Radio Radiance Field: The New Frontier of Spatial Wireless Channel Representation**  
  *IEEE Communications Magazine 2026* · [Paper](https://doi.org/10.1109/MCOM.001.2500161) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **RadioDUN: A Physics-Inspired Deep Unfolding Network for Radio Map Estimation**  
  *IEEE TCCN 2026* · [Paper](https://doi.org/10.1109/TCCN.2026.3683882) · <sub>JCR Q1 · CAS 2 · IF 8.0 · CCF None</sub>
- **RadioGen3D: 3D Radio Map Generation via Adversarial Learning on Large-Scale Synthetic Data**  
  *IEEE TVT 2026* · [Paper](https://doi.org/10.1109/TVT.2026.3724160) · <sub>JCR Q1 · CAS 2 · IF 7.5 · CCF None</sub>
- **RadioVAR: Fast Sampling-Free Radio Map Construction via Coarse-to-Fine Token Prediction**  
  *IEEE TCE 2026* · [Paper](https://ieeexplore.ieee.org/abstract/document/11493478) · <sub>JCR Q1 · CAS 2 · IF 9.9 · CCF None</sub>
- **Rank-Adaptive Bayesian Tensor Ring Completion for Low-Altitude 5D Radio Environment Map Construction**  
  *Big Data and Cognitive Computing 2026* · [Paper](https://doi.org/10.3390/bdcc10070220) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Rapid Generation of Channel Knowledge Map by Joint Physics and Conditional Diffusion Models**  
  *IEEE TNSE 2026* · [Paper](https://doi.org/10.1109/TNSE.2026.3720671) · <sub>JCR Q1 · CAS 2 · IF 7.3 · CCF None</sub>
- **REM-Net+: Quantified 3D Radio Environment Map Construction Guided by Radio Propagation Model**  
  *IEEE TVT 2026* · [Paper](https://doi.org/10.1109/TVT.2025.3623917) · <sub>JCR Q1 · CAS 2 · IF 7.5 · CCF None</sub>
- **RMF: one-step radio map reconstruction via mean flow matching**  
  *Connection Science 2026* · [Paper](https://www.tandfonline.com/doi/full/10.1080/09540091.2026.2658948) · <sub>JCR None · CAS None · IF None · CCF C</sub>
- **RMG-SRGAN: Super-Resolution Generative Adversarial Network based on multi-scale attention aggregation and feature enhancement for radio map generation**  
  *ICT Express 2026* · [Paper](https://www.sciencedirect.com/science/article/pii/S240595952600069X) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Scene Structure Based Neural Radio-Frequency Radiance Fields for Channel Knowledge Map Construction**  
  *IEEE WCL 2026* · [Paper](https://doi.org/10.1109/LWC.2025.3615621) · <sub>JCR Q1 · CAS 2 · IF 5.1 · CCF None</sub>
- **STFD-SNN: A Physics-Constrained Spiking Neural Network Framework for Maritime Radio Environment Map Reconstruction**  
  *IEEE IoT J 2026* · [Paper](https://doi.org/10.1109/JIOT.2026.3702982) · <sub>JCR Q1 · CAS 1 · IF 8.7 · CCF C</sub>
- **Topology-Aware Embedding Network for Label-Free Radio Map Construction**  
  *IEEE TWC 2026* · [Paper](https://doi.org/10.1109/TWC.2026.3713038) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **Toward Precise Channel Knowledge Map: Exploiting Environmental Information from 2D Visuals to 3D Point Clouds**  
  *IEEE Communications Magazine 2026* · [Paper](https://doi.org/10.1109/mcom.001.2500630) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **UniRM: A Universal Large Model for Multiband 3D Radio Map Construction**  
  *IEEE JSAC 2026* · [Paper](https://doi.org/10.1109/JSAC.2025.3640150) · <sub>JCR Q1 · CAS 1 · IF 16.8 · CCF A</sub>
- **VoxelRF: Voxelized Radiance Field for Fast Wireless Channel Modeling**  
  *IEEE CL 2026* · [Paper](https://doi.org/10.1109/lcomm.2025.3644410) · <sub>JCR Q2 · CAS 3 · IF 4.5 · CCF None</sub>

</details>

<details open>
<summary><strong>2025</strong> <sub>59 papers</sub></summary>

- **3-D Radio Map Estimation Based on Active Measurement Trajectory Selection**  
  *IEEE WCL 2025* · [Paper](https://doi.org/10.1109/LWC.2025.3557556) · <sub>JCR Q1 · CAS 2 · IF 5.1 · CCF None</sub>
- **3D-RadioDiff: An Altitude-Conditioned Diffusion Model for 3D Radio Map Construction**  
  *IEEE Wireless Communications Letters 2025* · [Paper](https://ieeexplore.ieee.org/document/10963917) · <sub>JCR Q1 · CAS 2 · IF 5.1 · CCF None</sub>
- **A Data-and-Semantic Dual-Driven Intelligent Inference Framework for Simultaneously Spectrum Map Construction and Signal Source Localization**  
  *IEEE IoT J 2025* · [Paper](https://ieeexplore.ieee.org/document/10946994) · <sub>JCR Q1 · CAS 1 · IF 8.7 · CCF C</sub>
- **A Data-driven Transfer Learning Method for Indoor Radio Map Estimation**  
  *IEEE Transactions on Vehicular Technology 2025* · [Paper](https://ieeexplore.ieee.org/document/11159327) · <sub>JCR Q1 · CAS 2 · IF 7.5 · CCF None</sub>
- **A Disentangled Representation Learning Framework for Low-altitude Network Coverage Prediction**  
  *IEEE Transactions on Mobile Computing 2025* · [Paper](https://ieeexplore.ieee.org/document/11268973) · <sub>JCR Q1 · CAS 1 · IF 8.8 · CCF A</sub>
- **A Recent Survey on Radio Map Estimation Methods for Wireless Networks**  
  *Electronics 2025* · [Paper](https://doi.org/10.3390/electronics14081564) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **A robust learning framework for spatial-temporal-spectral radio map prediction**  
  *Elsevier ESWA 2025* · [Paper](https://www.sciencedirect.com/science/article/abs/pii/S0957417425029665) · <sub>JCR None · CAS None · IF None · CCF C</sub>
- **A Sampling Method for Radio Environment Map Reconstruction Based on Multiarea Gudmundson Model**  
  *IEEE Sensors Journal 2025* · [Paper](https://doi.org/10.1109/jsen.2025.3559083) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **A Secure Wireless Transmission Scheme: Reconstructing Spatial Radio Environment Map and Redirecting Electromagnetic Signal Propagation Path**  
  *IEEE OJCOMS 2025* · [Paper](https://ieeexplore.ieee.org/document/11146515) · <sub>JCR Q1 · CAS 2 · IF 6.1 · CCF None</sub>
- **An Accurate Radio Environment Map Reconstruction Method**  
  *IEEE Sensors Journal 2025* · [Paper](https://doi.org/10.1109/JSEN.2025.3562214) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **An I2I Inpainting Approach for Efficient Channel Knowledge Map Construction**  
  *IEEE TWC 2025* · [Paper](https://ieeexplore.ieee.org/document/10791446) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **CF-CGN: Channel Fingerprints Extrapolation for Multi-band Massive MIMO Transmission based on Cycle-Consistent Generative Networks**  
  *IEEE JSAC 2025* · [Paper](https://ieeexplore.ieee.org/document/11059941) · <sub>JCR Q1 · CAS 1 · IF 16.8 · CCF A</sub>
- **Channel Gain Map Construction Based on Subregional Learning and Prediction**  
  *IEEE TVT 2025* · [Paper](https://ieeexplore.ieee.org/document/10891198) · <sub>JCR Q1 · CAS 2 · IF 7.5 · CCF None</sub>
- **Channel Gain Map Estimation for Wireless Networks Based on Scatterer Model**  
  *IEEE TWC 2025* · [Paper](https://doi.org/10.1109/TWC.2025.3557417) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **Channel Knowledge Map for Cellular-Connected UAV via Binary Bayesian Filtering**  
  *IEEE TCOM 2025* · [Paper](https://doi.org/10.1109/tcomm.2025.3554681) · <sub>JCR Q1 · CAS 2 · IF 8.4 · CCF B</sub>
- **CKMImageNet: A Dataset for AI-Based Channel Knowledge Map Toward Environment-Aware Communication and Sensing**  
  *IEEE TCOM 2025* · [Paper](https://doi.org/10.1109/TCOMM.2025.3615778) · <sub>JCR Q1 · CAS 2 · IF 8.4 · CCF B</sub>
- **Constructing Frequency Modulation-Broadcasting Map Based on Semi-Supervised Clustering**  
  *IEEE Transactions on Broadcasting 2025* · [Paper](https://ieeexplore.ieee.org/document/11074722) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Convolutional neural networks for predicting the perceived density of large urban fabrics**  
  *Elsevier CEUS 2025* · [Paper](https://www.sciencedirect.com/science/article/abs/pii/S0198971525000572) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **DeepRT: A Hybrid Framework Combining Large Model Architectures and Ray Tracing Principles for 6G Digital Twin Channels**  
  *MDPI Electronics 2025* · [Paper](https://www.mdpi.com/2079-9292/14/9/1849) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Denoising Diffusion Probabilistic Model for Radio Map Estimation in Generative Wireless Networks**  
  *IEEE TCCN 2025* · [Paper](https://ieeexplore.ieee.org/document/10843401) · <sub>JCR Q1 · CAS 2 · IF 8.0 · CCF None</sub>
- **Electromagnetic wave property inspired radio environment knowledge construction and artificial intelligence based verification for 6G digital twin channel**  
  *Frontiers of Information Technology & Electronic Engineering 2025* · [Paper](https://doi.org/10.1631/FITEE.2400464) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Fast Transmission Control Adaptation for URLLC via Channel Knowledge Map and Meta-Learning**  
  *IEEE Internet of Things Journal 2025* · [Paper](https://ieeexplore.ieee.org/document/10930931) · <sub>JCR Q1 · CAS 1 · IF 8.7 · CCF C</sub>
- **Generating CKM Using Others' Data: Cross-AP CKM Inference with Deep Learning**  
  *IEEE TVT 2025* · [Paper](https://ieeexplore.ieee.org/document/11146461) · <sub>JCR Q1 · CAS 2 · IF 7.5 · CCF None</sub>
- **Geo2ComMap: Deep Learning-Based MIMO Throughput Prediction Using Geographic Data**  
  *IEEE WCL 2025* · [Paper](https://ieeexplore.ieee.org/document/10950386) · <sub>JCR Q1 · CAS 2 · IF 5.1 · CCF None</sub>
- **GPRT: A Gaussian Process Regression-Based Radio Map Construction Method for Rugged Terrain**  
  *IEEE Internet of Things Journal 2025* · [Paper](https://ieeexplore.ieee.org/document/10938559) · <sub>JCR Q1 · CAS 1 · IF 8.7 · CCF C</sub>
- **Grid-Free Radio Map Estimation via Unsupervised Implicit Continuous Representation**  
  *IEEE SPL 2025* · [Paper](https://doi.org/10.1109/LSP.2025.3601038) · <sub>JCR None · CAS None · IF None · CCF C</sub>
- **GS3D: Signal Recovery in 3-D Wireless Networks With Gaussian Splatting-Synergized Stable Diffusion**  
  *IEEE WCL 2025* · [Paper](https://doi.org/10.1109/LWC.2025.3561522) · <sub>JCR Q1 · CAS 2 · IF 5.1 · CCF None</sub>
- **High-Efficiency Urban 3D Radio Map Estimation Based on Sparse Measurements**  
  *IEEE TVT 2025* · [Paper](https://doi.org/10.1109/TVT.2025.3573595) · <sub>JCR Q1 · CAS 2 · IF 7.5 · CCF None</sub>
- **IMNet: Interference-Aware Channel Knowledge Map Construction and Localization**  
  *IEEE WCL 2025* · [Paper](https://ieeexplore.ieee.org/document/10829585) · <sub>JCR Q1 · CAS 2 · IF 5.1 · CCF None</sub>
- **Interference Management in Private 5G Networks via Interference Source Localization and Radio Environment Map Construction**  
  *IEEE Access 2025* · [Paper](https://doi.org/10.1109/access.2025.3617014) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Interference-Cancellation-Based Channel Knowledge Map Construction and Its Applications to Channel Estimation**  
  *IEEE TWC 2025* · [Paper](https://doi.org/10.1109/TWC.2025.3552519) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **KAN Based Interpretable Radio Map Prediction Framework with Symbolic Data Fusion**  
  *IEEE Transactions on Cognitive Communications and Networking 2025* · [Paper](https://ieeexplore.ieee.org/document/11143592) · <sub>JCR Q1 · CAS 2 · IF 8.0 · CCF None</sub>
- **Large Language Model Agents for Radio Map Generation and Wireless Network Planning**  
  *IEEE Networking Letters 2025* · [Paper](https://doi.org/10.1109/lnet.2025.3539829) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Learnable Wireless Digital Twins: Reconstructing Electromagnetic Field With Neural Representations**  
  *IEEE OJ-COMS 2025* · [Paper](https://doi.org/10.1109/OJCOMS.2025.3535959) · <sub>JCR Q1 · CAS 2 · IF 6.1 · CCF None</sub>
- **Leveraging Transfer Learning for Radio Map Estimation via Mixture of Experts**  
  *IEEE TCCN 2025* · [Paper](https://ieeexplore.ieee.org/document/11005396) · <sub>JCR Q1 · CAS 2 · IF 8.0 · CCF None</sub>
- **Machine learning methods comparison for maritime wireless signal strength prediction**  
  *Elsevier JEngAppai 2025* · [Paper](https://www.sciencedirect.com/science/article/abs/pii/S0952197625013594) · <sub>JCR None · CAS None · IF None · CCF C</sub>
- **Machine-Learning-Based Path Loss Prediction for Vehicle-to-Vehicle Communication in Highway Environments**  
  *MDPI Applied Science 2025* · [Paper](https://www.mdpi.com/2076-3417/14/17/7545) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **MAE-Based Radio Map Construction for Wi-Fi Fingerprint Indoor Localization**  
  *IEEE CL 2025* · [Paper](https://doi.org/10.1109/lcomm.2025.3582075) · <sub>JCR Q2 · CAS 3 · IF 4.5 · CCF None</sub>
- **Novel Radio Environment Map Construction Scheme for 3-D and Full Band for Modern Internet of Things Applications**  
  *IEEE IoT J 2025* · [Paper](https://doi.org/10.1109/JIOT.2024.3520611) · <sub>JCR Q1 · CAS 1 · IF 8.7 · CCF C</sub>
- **Physics-Guided Language Model via Low-Rank Adaptation for Path Loss Prediction**  
  *IEEE Transactions on Cognitive Communications and Networking 2025* · [Paper](https://ieeexplore.ieee.org/document/11202166) · <sub>JCR Q1 · CAS 2 · IF 8.0 · CCF None</sub>
- **Prototyping and Experimental Results for ISAC-Based Channel Knowledge Map**  
  *IEEE TVT 2025* · [Paper](https://doi.org/10.1109/tvt.2025.3545785) · <sub>JCR Q1 · CAS 2 · IF 7.5 · CCF None</sub>
- **Radiation Source Localization Using Radio Maps: A Computer Vision Approach**  
  *IEEE Wireless Communications Letters 2025* · [Paper](https://ieeexplore.ieee.org/document/11137360) · <sub>JCR Q1 · CAS 2 · IF 5.1 · CCF None</sub>
- **Radio Environment Map Reconstruction via Tensor Completion: Bayesian and Semantic Approaches**  
  *IEEE TVT 2025* · [Paper](https://doi.org/10.1109/TVT.2025.3531124) · <sub>JCR Q1 · CAS 2 · IF 7.5 · CCF None</sub>
- **Radio map estimation using a CycleGAN-based learning framework for 6G wireless communication**  
  *Digital Communications and Networks 2025* · [Paper](https://www.sciencedirect.com/science/article/pii/S2352864825001294) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Radio Map Prediction from Aerial Images and Application to Coverage Optimization**  
  *IEEE TWC 2025* · [Paper](https://ieeexplore.ieee.org/document/11063460) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **Radio Map Reconstruction Based on Deep Denoising Regularization for UAV Communications**  
  *IEEE TVT 2025* · [Paper](https://doi.org/10.1109/TVT.2025.3542768) · <sub>JCR Q1 · CAS 2 · IF 7.5 · CCF None</sub>
- **Reconstruction of Radio Environment Map Based on Multi-Source Domain Adaptive of Graph Neural Network for Regression**  
  *MDPI Sensors 2025* · [Paper](https://www.mdpi.com/1424-8220/24/8/2523) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **RGB-D Sensor-Aided Radio Map Estimation Using Materials Classification**  
  *IEEE WCL 2025* · [Paper](https://doi.org/10.1109/LWC.2025.3557794) · <sub>JCR Q1 · CAS 2 · IF 5.1 · CCF None</sub>
- **SC-GAN: A spectrum cartography with satellite Internet based on Pix2Pix generative adversarial network**  
  *China Communications 2025* · [Paper](https://ieeexplore.ieee.org/document/10906396) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **ST-CSFNet: Spatial-Temporal Cross Scale FNet for Radio Environment Map Forecasting**  
  *IEEE CL 2025* · [Paper](https://doi.org/10.1109/LCOMM.2025.3529898) · <sub>JCR Q2 · CAS 3 · IF 4.5 · CCF None</sub>
- **Time-Variant Radio Map Reconstruction With Optimized Distributed Sensors in Dynamic Spectrum Environments**  
  *IEEE IoT J 2025* · [Paper](https://ieeexplore.ieee.org/document/10902472) · <sub>JCR Q1 · CAS 1 · IF 8.7 · CCF C</sub>
- **TiRE-GAN: Task-Incentivized Generative Learning for Radiomap Estimation**  
  *IEEE Wireless Communications Letters 2025* · [Paper](https://ieeexplore.ieee.org/document/10892295) · <sub>JCR Q1 · CAS 2 · IF 5.1 · CCF None</sub>
- **Towards accurate positioning: Directionality-enhanced fingerprinting radio map**  
  *ICT Express 2025* · [Paper](https://doi.org/10.1016/j.icte.2024.12.004) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Two-Stage Radio Map Construction With Real Environments and Sparse Measurements**  
  *IEEE WCL 2025* · [Paper](https://ieeexplore.ieee.org/document/10838708) · <sub>JCR Q1 · CAS 2 · IF 5.1 · CCF None</sub>
- **Unmanned Aerial Vehicle-Enabled Aerial Radio Environment Map Construction: A Multi-Stage Approach to Data Sampling and Path Planning**  
  *Drones 2025* · [Paper](https://doi.org/10.3390/drones9020081) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Vision Transformers for Efficient Indoor Pathloss Radio Map Prediction**  
  *MDPI Electronics 2025* · [Paper](https://www.mdpi.com/2079-9292/14/10/1905) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Visual transformer based unified framework for radio map estimation and optimized site selection**  
  *IEICE Transactions on Communications 2025* · [Paper](https://ieeexplore.ieee.org/document/11114848) · <sub>JCR Q1 · CAS 2 · IF 8.4 · CCF B</sub>
- **WiFi-Diffusion: Achieving Fine-Grained WiFi Radio Map Estimation with Ultra-Low Sampling Rate by Diffusion Models**  
  *IEEE JSAC 2025* · [Paper](https://ieeexplore.ieee.org/document/11066175) · <sub>JCR Q1 · CAS 1 · IF 16.8 · CCF A</sub>
- **WirelessNet: An Efficient Radio Access Network Model Based on Heterogeneous Graph Neural Networks**  
  *IEEE Access 2025* · [Paper](https://ieeexplore.ieee.org/document/10902045) · <sub>JCR None · CAS None · IF None · CCF None</sub>

</details>

<details open>
<summary><strong>2024 and Earlier</strong> <sub>33 papers</sub></summary>

- **A Deep Learning-Based Indoor Radio Estimation Method Driven by 2.4 GHz Ray-Tracing Data**  
  *IEEE Access 2023* · [Paper](https://ieeexplore.ieee.org/document/10347228) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **A Deep-Learning Approach to a Volumetric Radio Environment Map Construction for UAV-Assisted Networks**  
  *Wiley IJAP 2024* · [Paper](https://onlinelibrary.wiley.com/doi/full/10.1155/2024/9062023) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **A FL-Based Radio Map Reconstruction Approach for UAV-Aided Wireless Networks**  
  *MDPI Electronics 2023* · [Paper](https://www.mdpi.com/2079-9292/12/13/2817) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **A Graph Neural Network Based Radio Map Construction Method for Urban Environment**  
  *IEEE WLC 2023* · [Paper](https://ieeexplore.ieee.org/document/10078269) · <sub>JCR Q1 · CAS 2 · IF 5.1 · CCF None</sub>
- **A Novel Multimodal Fusion Sensing-Based Channel Prediction Method for UAV Communications**  
  *IEEE Internet of Things Journal 2024* · [Paper](https://ieeexplore.ieee.org/document/10720899) · <sub>JCR Q1 · CAS 1 · IF 8.7 · CCF C</sub>
- **A Scalable and Generalizable Pathloss Map Prediction**  
  *IEEE TWC 2024* · [Paper](https://ieeexplore.ieee.org/document/10682525) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **A Tutorial on Environment-Aware Communications via Channel Knowledge Map for 6G**  
  *IEEE Communications Surveys & Tutorials 2024* · [Paper](https://ieeexplore.ieee.org/document/10430216) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Accurate Spectrum Map Construction for Spectrum Management Through Intelligent Frequency-Spatial Reasoning**  
  *IEEE TCOMM 2023* · [Paper](https://ieeexplore.ieee.org/document/10103465) · <sub>JCR Q1 · CAS 2 · IF 8.4 · CCF B</sub>
- **An Empirical Study on Using CNNs for Fast Radio Signal Prediction**  
  *Springer SN Computer Science 2022* · [Paper](https://link.springer.com/article/10.1007/s42979-022-01022-2) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **An Overview of Propagation Models BasElectriacaled on Deep Learning Techniques**  
  *International Journal Electrical Engineering 2022* · [Paper](https://ijeec.etf.ues.rs.ba/index.php/ijeec/article/view/134) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Cell-Level RSRP Estimation With the Image-to-Image Wireless Propagation Model Based on Measured Data**  
  *IEEE TCCN 2023* · [Paper](https://ieeexplore.ieee.org/document/10227351) · <sub>JCR Q1 · CAS 2 · IF 8.0 · CCF None</sub>
- **Channel Path Loss Prediction Using Satellite Images: A Deep Learning Approach**  
  *IEEE TMLCN 2024* · [Paper](https://ieeexplore.ieee.org/document/10663692) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Deep Learning for Reduced Sampling Spatial 3-D REM Reconstruction**  
  *IEEE OJCOMS 2024* · [Paper](https://ieeexplore.ieee.org/document/10495350) · <sub>JCR Q1 · CAS 2 · IF 6.1 · CCF None</sub>
- **Deep Learning-Empowered RF Sensing in Outdoor Environments: Recent Advances, Challenges, and Future Directions**  
  *MDPI Electronics 2024* · [Paper](https://www.mdpi.com/2079-9292/14/1/125) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Deep-Learning-Based Radio Map Reconstruction for V2X Communications**  
  *IEEE TVT 2024* · [Paper](https://ieeexplore.ieee.org/document/10292913) · <sub>JCR Q1 · CAS 2 · IF 7.5 · CCF None</sub>
- **DeepREM: Deep-Learning-Based Radio Environment Map Estimation From Sparse Measurements**  
  *IEEE Access 2023* · [Paper](https://ieeexplore.ieee.org/document/10127968) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Development of a Conditional Generative Adversarial Network Model for Television Spectrum Radio Environment Mapping**  
  *IEEE Access 2024* · [Paper](https://ieeexplore.ieee.org/document/10813343) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Diffraction and Scattering Aware Radio Map and Environment Reconstruction Using Geometry Model-Assisted Deep Learning**  
  *IEEE Transactions on Wireless Communications 2024* · [Paper](https://ieeexplore.ieee.org/document/10745208) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **Fast Radio Propagation Prediction with Deep Learning**  
  *Compressed Sensing in Information Processing 2022* · [Paper](https://link.springer.com/chapter/10.1007/978-3-031-09745-4_10) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Intelligent reconstruction algorithm of electromagnetic map based on propagation model**  
  *Journal of Communications and Networks 2024* · [Paper](https://ieeexplore.ieee.org/document/10748590) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Machine Learning for Channel Quality Prediction: From Concept to Experimental Validation**  
  *IEEE Transactions on Wireless Communications 2024* · [Paper](https://ieeexplore.ieee.org/document/10577598) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **Machine Learning-Based Radio Coverage Prediction in Urban Environments**  
  *IEEE TNSE 2020* · [Paper](https://ieeexplore.ieee.org/document/9247298) · <sub>JCR Q1 · CAS 2 · IF 7.3 · CCF None</sub>
- **Machine Learning-Based Urban Canyon Path Loss Prediction Using 28 GHz Manhattan Measurements**  
  *IEEE TAP 2022* · [Paper](https://ieeexplore.ieee.org/document/9722715) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Multi-Stage RF Emitter Search and Geolocation With UAV: A Cognitive Learning-Based Method**  
  *IEEE Transactions on Vehicular Technology 2023* · [Paper](https://ieeexplore.ieee.org/document/10016683) · <sub>JCR Q1 · CAS 2 · IF 7.5 · CCF None</sub>
- **Overview on IEEE 802.11bf: WLAN Sensing**  
  *IEEE Communications Surveys & Tutorials 2024* · [Paper](https://ieeexplore.ieee.org/document/10547188) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Physics-Informed Neural Networks for Path Loss Estimation by Solving Electromagnetic Integral Equations**  
  *IEEE TWC 2024* · [Paper](https://ieeexplore.ieee.org/document/10608081) · <sub>JCR Q1 · CAS 1 · IF 10.3 · CCF B</sub>
- **Pseudo Ray-Tracing: Deep Leaning Assisted Outdoor mm-Wave Path Loss Prediction**  
  *IEEE WLC 2022* · [Paper](https://ieeexplore.ieee.org/document/9774859) · <sub>JCR Q1 · CAS 2 · IF 5.1 · CCF None</sub>
- **Radiomap Inpainting for Restricted Areas Based on Propagation Priority and Depth Map**  
  *IEEE TNSE 2020* · [Paper](https://ieeexplore.ieee.org/document/10424680) · <sub>JCR Q1 · CAS 2 · IF 7.3 · CCF None</sub>
- **REM-U-Net: Deep Learning Based Agile REM Prediction With Energy-Efficient Cell-Free Use Case**  
  *IEEE OJSP 2024* · [Paper](https://ieeexplore.ieee.org/document/10474197) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Rigorous Indoor Wireless Communication System Simulations With Deep Learning-Based Radio Propagation Models**  
  *IEEE JMMCT 2024* · [Paper](https://ieeexplore.ieee.org/document/10786819) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Super-Resolution of Wireless Channel Characteristics: A Multitask Learning Model**  
  *IEEE TAP 2023* · [Paper](https://ieeexplore.ieee.org/document/10224827) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Temporal prediction for spectrum environment maps with moving radiation sources**  
  *IET Communications 2023* · [Paper](https://ietresearch.onlinelibrary.wiley.com/doi/full/10.1049/cmu2.12560) · <sub>JCR None · CAS None · IF None · CCF C</sub>
- **Weighted Ensembles for Adaptive Active Learning**  
  *IEEE TSP 2024* · [Paper](https://ieeexplore.ieee.org/document/10648946) · <sub>JCR Q1 · CAS 1 · IF 5.5 · CCF None</sub>

</details>

<details open>
<summary><strong>Year Unspecified</strong> <sub>2 papers</sub></summary>

- **A graph neural network-based method for 3D radio environment map construction in urban scenarios**  
  *IEEE JSEE* · [Paper](https://ieeexplore.ieee.org/abstract/document/11456877) · <sub>JCR None · CAS None · IF None · CCF None</sub>
- **Minimum energy consumption of multi-UAV logistics distribution with heading angle constraints based on radio map**  
  *J. PhyCom* · [Paper](https://www.sciencedirect.com/science/article/abs/pii/S1874490726001382) · <sub>JCR None · CAS None · IF None · CCF None</sub>

</details>

### Conferences

<details open>
<summary><strong>2026</strong> <sub>35 papers</sub></summary>

- **3D Dynamic Radio Map Prediction Using Vision Transformers for Low-Altitude Wireless Networks**  
  *IEEE ICC 2026* · [Paper](https://doi.org/10.1109/ICC59461.2026.11587465) · <sub>CCF C</sub>
- **3D Radio Environment Map Reconstruction Under Sparse Sampling Scenario via Bayesian Neural Fields**  
  *IEEE CCAI 2026* · [Paper](https://doi.org/10.1109/CCAI69603.2026.11641967) · <sub>CCF None</sub>
- **3DCKM: A Large-Scale Multiband 3D Channel Knowledge Map Dataset for Low-Altitude Corridors**  
  *IEEE ICC Workshops 2026* · [Paper](https://doi.org/10.1109/ICCWORKSHOPS63917.2026.11586209) · <sub>CCF None</sub>
- **A Data-Friendly Deep Learning Architecture for High-Resolution Radio Map Construction**  
  *IEEE ICC Workshops 2026* · [Paper](https://doi.org/10.1109/ICCWORKSHOPS63917.2026.11586443) · <sub>CCF None</sub>
- **A Seven-Channel Encoder-Decoder Network for Radio Map Estimation in Wireless Network Planning**  
  *IEEE EICCT 2026* · [Paper](https://doi.org/10.1109/eicct69950.2026.11564565) · <sub>CCF None</sub>
- **Accurate Radio Map Estimation from Sparse Measurements with U-Nets**  
  *EuCAP 2026* · [Paper](https://doi.org/10.23919/EUCAP68105.2026.11612118) · <sub>CCF None</sub>
- **BeamCKMDiff: Beam-Aware Channel Knowledge Map Construction via Diffusion Transformer**  
  *IEEE INFOCOM 2026* · [Paper](https://doi.org/10.1109/INFOCOM59046.2026.11571730) · <sub>CCF A</sub>
- **Channel Gain Map Reconstruction Based on Virtual Scatterer Model**  
  *IEEE ICC 2026* · [Paper](https://doi.org/10.1109/ICC59461.2026.11587260) · <sub>CCF C</sub>
- **Compressed Spectrum Cartography: Estimating Wide-Band Channel Gain Maps from Sub-Nyquist Delay Correlations**  
  *IEEE ICASSP 2026* · [Paper](https://ieeexplore.ieee.org/abstract/document/11462622) · <sub>CCF B</sub>
- **Constructing Knowledge Map for MIMO-OFDM Clustered Channel Estimation**  
  *IEEE ICC 2026* · [Paper](https://doi.org/10.1109/icc59461.2026.11587611) · <sub>CCF C</sub>
- **CR-DDPM: A Correction-Refined Diffusion Model for Sampling-Based Radio Map Construction**  
  *ACM 2026* · [Paper](https://doi.org/10.1145/3804601.3804674) · <sub>CCF None</sub>
- **Diffusion Model for Channel Knowledge Map Construction and Channel Estimation**  
  *IEEE ICTC 2026* · [Paper](https://doi.org/10.1109/ictc70246.2026.11650045) · <sub>CCF None</sub>
- **Efficient Clustered Channel Estimation via Channel Knowledge Map Constructed with Tensor Decomposition**  
  *IEEE ICC 2026* · [Paper](https://doi.org/10.1109/icc59461.2026.11587011) · <sub>CCF C</sub>
- **Energy-Efficient Channel Knowledge Map Construction for 6G Networks Using Improved Transformer**  
  *IEEE EMC2 2026* · [Paper](https://doi.org/10.1109/emc68537.2026.11441666) · <sub>CCF None</sub>
- **Flow Matching-Based Active Learning for Radio Map Construction with Low-Altitude UAVs**  
  *IEEE ICASSP 2026* · [Paper](https://doi.org/10.1109/ICASSP55912.2026.11463842) · <sub>CCF B</sub>
- **FM-RME: Foundation Model Empowered Radio Map Estimation**  
  *IEEE ICC 2026* · [Paper](https://doi.org/10.1109/ICC59461.2026.11586874) · <sub>CCF C</sub>
- **Knowledge-Infused Diffusion Model for Effective Radio Spectrum Map Construction**  
  *IEEE ICC Workshops 2026* · [Paper](https://doi.org/10.1109/ICCWORKSHOPS63917.2026.11586366) · <sub>CCF None</sub>
- **Location-Agnostic Channel Knowledge Map Construction for Dynamic Scenes**  
  *IEEE ICC 2026* · [Paper](https://doi.org/10.1109/ICC59461.2026.11587910) · <sub>CCF C</sub>
- **Multiantenna Channel Map Prediction with Missing Location Information using Contrastive Learning and Graph Neural Networks**  
  *IEEE ICASSP 2026* · [Paper](https://ieeexplore.ieee.org/abstract/document/11462797) · <sub>CCF B</sub>
- **Neural Network Parameterized Bayesian Nonstationary Radio Map Estimation With Uncertain Location**  
  *IEEE INFOCOM 2026* · [Paper](https://doi.org/10.1109/INFOCOM59046.2026.11571402) · <sub>CCF A</sub>
- **oneTwin: Online Digital Network Twin via Neural Radio Radiance Field**  
  *IEEE INFOCOM 2026* · [Paper](https://doi.org/10.1109/INFOCOM59046.2026.11571503) · <sub>CCF A</sub>
- **PAUnet: A Novel Position Attention-Based Model for Radio Map Construction**  
  *IEEE CNML 2026* · [Paper](https://doi.org/10.1109/cnml68938.2026.11452971) · <sub>CCF None</sub>
- **Physical Embedding for Radio Map Construction**  
  *IEEE ICC 2026* · [Paper](https://doi.org/10.1109/ICC59461.2026.11588289) · <sub>CCF C</sub>
- **Physical-Informed Radio Map Estimation: An Environment-Aware Decomposition Approach**  
  *IEEE WCNC Workshops 2026* · [Paper](https://doi.org/10.1109/wcncw67598.2026.11555749) · <sub>CCF None</sub>
- **Physics-Informed AI-Driven Cross-Frequency Channel Knowledge Map Construction**  
  *IEEE ICC 2026* · [Paper](https://doi.org/10.1109/ICC59461.2026.11586952) · <sub>CCF C</sub>
- **PIMV-GNN: A Physics-Informed Multi-View Graph Neural Network for Robust Channel Knowledge Map Construction**  
  *IEEE ICC 2026* · [Paper](https://doi.org/10.1109/ICC59461.2026.11588150) · <sub>CCF C</sub>
- **Planar Gaussian Splatting with Bilinear Spatial Transformer for Wireless Radiance Field Reconstruction**  
  *IEEE ICC Workshops 2026* · [Paper](https://doi.org/10.1109/ICCWORKSHOPS63917.2026.11586655) · <sub>CCF None</sub>
- **PropSplat: Map-Free RF Field Reconstruction via 3D Gaussian Propagation Splatting**  
  *IEEE DySPAN 2026* · [Paper](https://doi.org/10.1109/DYSPAN69846.2026.11571104) · <sub>CCF None</sub>
- **Radio Map Construction Using Graph Neural Network with Adaptive Sampling**  
  *IEEE CCAI 2026* · [Paper](https://doi.org/10.1109/ccai69603.2026.11642007) · <sub>CCF None</sub>
- **RadioAR: Autoregressive Modeling for Accurate Radio Map Estimation**  
  *ACM Web Companion 2026* · [Paper](https://doi.org/10.1145/3774905.3794674) · <sub>CCF None</sub>
- **RadioVIL: Anomaly-Aware Diffusion Models for Radio Map Inpainting and Zero-Shot Vehicle Localization**  
  *IEEE GLOBECOM 2026* · [Paper](https://arxiv.org/abs/2608.16167) · <sub>CCF C</sub>
- **Ripple-Inspired In-Context Learning for Radio Map Estimation**  
  *IEEE ICC 2026* · [Paper](https://doi.org/10.1109/ICC59461.2026.11587713) · <sub>CCF C</sub>
- **Robust Pathloss Radio Map Prediction via Physics-Informed Transformer**  
  *IEEE DySPAN 2026* · [Paper](https://doi.org/10.1109/DYSPAN69846.2026.11571154) · <sub>CCF None</sub>
- **SymNet: A Multi-Task Network for Joint Radio Map Reconstruction and Transmitter Localization**  
  *IEEE WACV 2026* · [Paper](https://doi.org/10.1109/WACV61042.2026.00023) · <sub>CCF None</sub>
- **TransfoREM: Transformer-Aided 3D Radio Environment Mapping**  
  *IEEE ICC 2026* · [Paper](https://doi.org/10.1109/ICC59461.2026.11587344) · <sub>CCF C</sub>

</details>

<details open>
<summary><strong>2025</strong> <sub>66 papers</sub></summary>

- **3D Urban Radio Map Estimation Based on UAV Sparse Measurement Data**  
  *IEEE ICCC 2025* · [Paper](https://doi.org/10.1109/iccc68654.2025.11437905) · <sub>CCF None</sub>
- **A Diffusion Framework for Accurate Fine-Grained Radio Map Reconstruction**  
  *IEEE GLOBECOM 2025* · [Paper](https://doi.org/10.1109/GLOBECOM59602.2025.11432702) · <sub>CCF C</sub>
- **A Diffusion-Based Propagation Model for Path Loss Prediction in Indoor Environments**  
  *2025 EuCAP* · [Paper](https://ieeexplore.ieee.org/document/10999434) · <sub>CCF None</sub>
- **A Gaussian Splatting Approach to Continuous Radio Map Construction**  
  *IEEE INFOCOM Workshops 2025* · [Paper](https://doi.org/10.1109/INFOCOMWKSHPS65812.2025.11153005) · <sub>CCF None</sub>
- **A Graph Neural Network-Based Radio Map Construction With Uncertainty Prediction**  
  *IEEE Asilomar 2025* · [Paper](https://doi.org/10.1109/ieeeconf67917.2025.11443759) · <sub>CCF None</sub>
- **A Radio Map Inpainting Construction Method for Dynamic Scenarios**  
  *IEEE VTC-Spring 2025* · [Paper](https://doi.org/10.1109/vtc2025-spring65109.2025.11174847) · <sub>CCF None</sub>
- **Adaptive Sampling for Online UAV-Aided Radio Map Sensing**  
  *EUSIPCO 2025* · [Paper](https://doi.org/10.23919/eusipco63237.2025.11226752) · <sub>CCF None</sub>
- **AIGC-Based Radio Map Construction for Channel Estimation in Low-Altitude Economy**  
  *IEEE/CIC ICCC Workshops 2025* · [Paper](https://doi.org/10.1109/icccworkshops67136.2025.11148098) · <sub>CCF None</sub>
- **An Improved Radio Environment Map Reconstruction Method Based on Gaussian Kernel Function Weight Driven by Mixed Models**  
  *IEEE ICCC 2025* · [Paper](https://doi.org/10.1109/iccc68654.2025.11437624) · <sub>CCF None</sub>
- **An Incremental Learning-Based Mechanism to Deploying Radio Map Estimation Models**  
  *IEEE ECIS 2025* · [Paper](https://doi.org/10.1109/ecis65594.2025.11087069) · <sub>CCF None</sub>
- **Analysis of Channel Knowledge Map Updating from Information Perspective**  
  *IEEE/CIC ICCC Workshops 2025* · [Paper](https://doi.org/10.1109/icccworkshops67136.2025.11148189) · <sub>CCF None</sub>
- **Bayesian Radio Map Estimation: Fundamentals and Implementation via Diffusion Models**  
  *IEEE GLOBECOM Workshops 2025* · [Paper](https://doi.org/10.1109/GCWKSHPS68340.2025.11591154) · <sub>CCF None</sub>
- **Bayesian-based Transmission and Channel Knowledge Map Reconstruction in LEO Satellite-terrestrial Communication**  
  *IEEE/CIC ICCC Workshops 2025* · [Paper](https://doi.org/10.1109/icccworkshops67136.2025.11148227) · <sub>CCF None</sub>
- **Bayesian-Driven Graph Reasoning for Active Radio Map Construction**  
  *IEEE WCSP 2025* · [Paper](https://doi.org/10.1109/wcsp68525.2025.1010321) · <sub>CCF None</sub>
- **Beamforming-Codebook-Aware Channel Knowledge Map Construction for Multi-Antenna Systems**  
  *IEEE GLOBECOM 2025* · [Paper](https://doi.org/10.1109/globecom59602.2025.11432134) · <sub>CCF C</sub>
- **CFGRMDiff: A Conditional Diffusion Model for Radio Map Reconstruction**  
  *IEEE CCPQT 2025* · [Paper](https://doi.org/10.1109/ccpqt66408.2025.11383293) · <sub>CCF None</sub>
- **Channel Knowledge Map Updating with Machine Unlearning**  
  *IEEE/CIC ICCC Workshops 2025* · [Paper](https://doi.org/10.1109/icccworkshops67136.2025.11148163) · <sub>CCF None</sub>
- **Channel-Aware Deep Learning for Superimposed Pilot Power Allocation and Receiver Design**  
  *2025 VTC2025-Spring* · [Paper](https://ieeexplore.ieee.org/document/11174787) · <sub>CCF None</sub>
- **CNN-Based Channel Map Estimation for Movable Antenna Systems**  
  *IEEE SPAWC 2025* · [Paper](https://doi.org/10.1109/spawc66079.2025.11143328) · <sub>CCF None</sub>
- **Constructing 4D Radio Map in LEO Satellite Networks with Limited Samples**  
  *IEEE INFOCOM 2025* · [Paper](https://doi.org/10.1109/INFOCOM55648.2025.11044496) · <sub>CCF A</sub>
- **Deep Learning for Radio Map Estimation in Wireless Communication Networks**  
  *ACM AISNS 2025* · [Paper](https://doi.org/10.1145/3797161.3797212) · <sub>CCF None</sub>
- **Deep Learning-Based CKM Construction with Image Super-Resolution**  
  *IEEE VTC-Spring 2025* · [Paper](https://ieeexplore.ieee.org/document/11174933) · <sub>CCF None</sub>
- **Differentiable Channel Knowledge Map Reconstruction via Kolmogorov-Arnold Networks**  
  *IEEE WCSP 2025* · [Paper](https://doi.org/10.1109/wcsp68525.2025.1010403) · <sub>CCF None</sub>
- **DULRTC-RME: A Deep Unrolled Low-rank Tensor Completion Network for Radio Map Estimation**  
  *2025 ICASSP* · [Paper](https://ieeexplore.ieee.org/document/10889162) · <sub>CCF B</sub>
- **Energy-optimized Multilayer-Perception-Kriging channel map reconstruction method**  
  *ACM CCIP 2025* · [Paper](https://doi.org/10.1145/3784833.3784865) · <sub>CCF None</sub>
- **Environment-Aware AoD and AoA Prediction for Wireless Networks Utilizing Machine Learning**  
  *2025 ICAIIC* · [Paper](https://ieeexplore.ieee.org/document/10920688) · <sub>CCF None</sub>
- **FedRME: Importance-Aware Cooperative Radio Map Estimation Empowered by Vertical Federated Learning**  
  *2025 ICC Workshops* · [Paper](https://ieeexplore.ieee.org/document/11162141) · <sub>CCF None</sub>
- **FiReT: A Neural Radiance Fields Framework for Wireless Field Reconstruction and Transmitter Placement**  
  *IEEE IKT 2025* · [Paper](https://doi.org/10.1109/ikt69652.2025.11626679) · <sub>CCF None</sub>
- **Generative CKM construction using partially observed data with diffusion model**  
  *IEEE VTC-Spring 2025* · [Paper](https://ieeexplore.ieee.org/document/11174670) · <sub>CCF None</sub>
- **GenRadio: A Generative Framework for Fine-Grained 3D Radio Map Estimation**  
  *IEEE VTC-Fall 2025* · [Paper](https://doi.org/10.1109/VTC2025-FALL65116.2025.11309879) · <sub>CCF None</sub>
- **IPP-Net: A Generalizable Deep Neural Network Model for Indoor Pathloss Radio Map Prediction**  
  *IEEE ICASSP 2025* · [Paper](https://ieeexplore.ieee.org/document/10890663) · <sub>CCF B</sub>
- **IRM-Net: An Enhanced Attention Networks for Indoor Radio Map Estimation**  
  *IEEE MLSP 2025* · [Paper](https://doi.org/10.1109/mlsp62443.2025.11204298) · <sub>CCF None</sub>
- **Land Feature Aware Radio Environment Map Construction using Radio Oriented Heterogeneous Multitask Gaussian Process**  
  *IEEE GLOBECOM 2025* · [Paper](https://doi.org/10.1109/globecom59602.2025.11432161) · <sub>CCF C</sub>
- **Large-Scale Channel Modeling for UAV Base Station: A Radio Map Method**  
  *IEEE ICCT 2025* · [Paper](https://doi.org/10.1109/icct67417.2025.11374046) · <sub>CCF None</sub>
- **Learning Blockage and Reflection Geometry for MIMO Beam Map Construction**  
  *ICC 2025 - IEEE International Conference on Communications* · [Paper](https://ieeexplore.ieee.org/document/11161713) · <sub>CCF C</sub>
- **Multi-Modal Neural Radio Radiance Field for Localized Statistical Channel Modelling**  
  *IEEE GLOBECOM Workshops 2025* · [Paper](https://doi.org/10.1109/GCWKSHPS68340.2025.11591021) · <sub>CCF None</sub>
- **Multi-Path Aware Radio Map Construction for 6G Environment-Aware Communication: A Helmholtz Equation-Informed Approach**  
  *IEEE/CIC ICCC 2025* · [Paper](https://doi.org/10.1109/iccc65529.2025.11148641) · <sub>CCF None</sub>
- **NeRF-APT: A New NeRF Framework for Wireless Channel Prediction**  
  *IEEE INFOCOM Workshops 2025* · [Paper](https://doi.org/10.1109/INFOCOMWKSHPS65812.2025.11152993) · <sub>CCF None</sub>
- **Online Gaussian Process for Dynamic Radio Map Updating**  
  *IEEE MLSP 2025* · [Paper](https://doi.org/10.1109/mlsp62443.2025.11204325) · <sub>CCF None</sub>
- **Optimal Distance-Constrained Path Planning for Sparse Radio Map Recovery**  
  *IEEE VTC-Fall 2025* · [Paper](https://doi.org/10.1109/vtc2025-fall65116.2025.11310537) · <sub>CCF None</sub>
- **Physics-Informed Diffusion Model for Radio Environment Map Reconstruction from Sparse Measurements**  
  *IEEE APCC 2025* · [Paper](https://doi.org/10.23919/APCC64555.2025.11279913) · <sub>CCF None</sub>
- **Physics-Inspired Distributed Radio Map Estimation**  
  *IEEE ICC 2025* · [Paper](https://doi.org/10.1109/ICC52391.2025.11161293) · <sub>CCF C</sub>
- **Radio Environment Map Construction Method for Complex Scenarios: The Combination of Concentric Circle Algorithm and Detection Algorithm of Unknown Transmit Sources**  
  *IEEE ISCTIS 2025* · [Paper](https://doi.org/10.1109/isctis65944.2025.11065184) · <sub>CCF None</sub>
- **Radio Environment Map Reconstruction via Multi-Scale Feature Fusion Convolutional Autoencoder**  
  *IEEE ISEMC 2025* · [Paper](https://doi.org/10.1109/isemc68048.2025.11291887) · <sub>CCF None</sub>
- **Radio Map Estimation via Latent-Domain Plug-and-Play Denoisers**  
  *IEEE ICASSP 2025* · [Paper](https://ieeexplore.ieee.org/document/10889056) · <sub>CCF B</sub>
- **Radio Map Interpolation Using Geodesics on Statistical Manifolds**  
  *IEEE SmartIoT 2025* · [Paper](https://doi.org/10.1109/smartiot66867.2025.00011) · <sub>CCF None</sub>
- **Radio Map Reconstruction Based on Nas Enhanced Deep Regularization Completion for Uav Communications**  
  *2025 VTC2025-Spring* · [Paper](https://ieeexplore.ieee.org/document/11174759) · <sub>CCF None</sub>
- **RadioDiff-Turbo: Lightweight Generative Large Electromagnetic Model for Wireless Digital Twin Construction**  
  *IEEE INFOCOM wksp 2025* · [Paper](https://ieeexplore.ieee.org/document/11152929) · <sub>CCF None</sub>
- **RadioVAE: Generating Probabilistic Radio Map via Variational Autoencoder with UNet**  
  *IEEE GLOBECOM 2025* · [Paper](https://doi.org/10.1109/GLOBECOM59602.2025.11432712) · <sub>CCF C</sub>
- **RMTransformer: Accurate Radio Map Construction and Coverage Prediction**  
  *IEEE VTC-Spring 2025* · [Paper](https://doi.org/10.1109/VTC2025-SPRING65109.2025.11174709) · <sub>CCF None</sub>
- **Scalable AI-assisted optical radio environment map estimation for indoor visible-light communication systems (AI-VLCmaps)**  
  *SPIE Applications of Machine Learning 2025* · [Paper](https://doi.org/10.1117/12.3065355) · <sub>CCF None</sub>
- **Self-Supervised Learning Informed Radio Environment Map Estimation with Few Samples**  
  *IEEE GLOBECOM 2025* · [Paper](https://doi.org/10.1109/GLOBECOM59602.2025.11431663) · <sub>CCF C</sub>
- **Spatial Frequency Interleaving Residual Autoencoder for Indoor Radio Map Reconstruction**  
  *IEEE ICASSP 2025* · [Paper](https://doi.org/10.1109/ICASSP49660.2025.10888142) · <sub>CCF B</sub>
- **Spatial Transformers for Radio Map Estimation**  
  *IEEE ICC 2025* · [Paper](https://ieeexplore.ieee.org/document/11160709) · <sub>CCF C</sub>
- **Spatially-Aware Conformalized Quantile Regression for Radio Map Uncertainty Quantification**  
  *IEEE ICSPS 2025* · [Paper](https://doi.org/10.1109/icsps66615.2025.11347781) · <sub>CCF None</sub>
- **Supporting Urban Low-Altitude Economy: Channel Knowledge Map Inference Based on 3D Conditional GAN**  
  *IEEE GLOBECOM 2025* · [Paper](https://doi.org/10.1109/gcwkshps68340.2025.11590846) · <sub>CCF C</sub>
- **Terahertz Spatial Wireless Channel Modeling with Radio Radiance Field**  
  *IEEE GLOBECOM 2025* · [Paper](https://doi.org/10.1109/GLOBECOM59602.2025.11432624) · <sub>CCF C</sub>
- **The First Indoor Pathloss Radio Map Prediction Challenge**  
  *IEEE ICASSP 2025* · [Paper](https://doi.org/10.1109/ICASSP49660.2025.10889381) · <sub>CCF B</sub>
- **The Sampling-Assisted Pathloss Radio Map Prediction Competition**  
  *IEEE MLSP 2025* · [Paper](https://doi.org/10.1109/mlsp62443.2025.11204278) · <sub>CCF None</sub>
- **Trajectory Optimization for Channel Knowledge Map Construction in Low-Altitude UAV Systems**  
  *IEEE/CIC ICCC Workshops 2025* · [Paper](https://doi.org/10.1109/icccworkshops67136.2025.11148141) · <sub>CCF None</sub>
- **U-Net Based Indoor Radio Map Prediction Under Sparse Sampling**  
  *IEEE MLSP 2025* · [Paper](https://doi.org/10.1109/mlsp62443.2025.11204227) · <sub>CCF None</sub>
- **Ultra-Grained Channel Fingerprint Construction via Conditional Generative Diffusion Models**  
  *IEEE INFOCOM 2025* · [Paper](https://ieeexplore.ieee.org/document/11152821) · <sub>CCF A</sub>
- **UNet-Based Deep Learning Pathloss Estimator with Boundary Condition Input**  
  *2025 RWS* · [Paper](https://ieeexplore.ieee.org/document/10904839) · <sub>CCF None</sub>
- **Unsupervised Radio Map Construction in Mixed LoS/NLoS Indoor Environments**  
  *IEEE GLOBECOM 2025* · [Paper](https://doi.org/10.1109/GLOBECOM59602.2025.11432665) · <sub>CCF C</sub>
- **ViT-RefineNet for Directional Signal Radio Map Reconstruction from Sparse Samples**  
  *ACM SIGSPATIAL 2025* · [Paper](https://doi.org/10.1145/3748636.3762750) · <sub>CCF C</sub>
- **wGNN-Based Sparse Channel Angle Map Construction for MIMO Downlink Systems**  
  *IEEE ICCBD+AI 2025* · [Paper](https://doi.org/10.1109/iccbdai66607.2025.11388900) · <sub>CCF None</sub>

</details>

<details open>
<summary><strong>2024 and Earlier</strong> <sub>48 papers</sub></summary>

- **A 2D Deep Residual Learning Approach for 3D Indoor Radio Map Estimation**  
  *ICC 2024 - IEEE International Conference on Communications* · [Paper](https://ieeexplore.ieee.org/document/10622880) · <sub>CCF C</sub>
- **A Bayesian Learning Approach to Wireless Outdoor Heatmap Construction Using Deep Gaussian Process**  
  *2024 58th Asilomar Conference on Signals, Systems, and Computers* · [Paper](https://ieeexplore.ieee.org/document/10942780) · <sub>CCF None</sub>
- **A Multi-Task Learning Model for Super Resolution of Wireless Channel Characteristics**  
  *IEEE GlobeCom 2022* · [Paper](https://ieeexplore.ieee.org/document/10001700) · <sub>CCF C</sub>
- **A New Approach to Predict Radio Map via Learning-Based Spatial Loss Field**  
  *2024 ICASSPW* · [Paper](https://ieeexplore.ieee.org/document/10627524) · <sub>CCF None</sub>
- **A Transformer-Based Network for Unifying Radio Map Estimation and Optimized Site Selection**  
  *2024 ICASSPW* · [Paper](https://ieeexplore.ieee.org/document/10627516) · <sub>CCF None</sub>
- **Agile Radio Map Prediction Using Deep Learning**  
  *IEEE ICASSP 2023* · [Paper](https://ieeexplore.ieee.org/document/10096546) · <sub>CCF B</sub>
- **Channel Knowledge Map Construction with Laplacian Pyramid Reconstruction Network**  
  *IEEE WCNC 2024* · [Paper](https://ieeexplore.ieee.org/document/10570885) · <sub>CCF C</sub>
- **Channel Knowledge Maps Construction Based on Point Cloud Environment Information**  
  *IEEE VTC-Fall 2024* · [Paper](https://ieeexplore.ieee.org/document/10757755) · <sub>CCF None</sub>
- **Data-and-Semantic Dual-Driven Spectrum Map Construction for 6G Spectrum Management**  
  *IEEE GlobeCom 2024* · [Paper](https://ieeexplore.ieee.org/document/10901421) · <sub>CCF C</sub>
- **Data-Driven Radio Environment Map Estimation Using Graph Neural Networks**  
  *IEEE ICC wksp 2024* · [Paper](https://ieeexplore.ieee.org/document/10615637) · <sub>CCF None</sub>
- **Deep Learning-Based Path Loss Prediction for Outdoor Wireless Communication Systems**  
  *IEEE ICASSP 2023* · [Paper](https://ieeexplore.ieee.org/document/10095501) · <sub>CCF B</sub>
- **Deep Learning-Based Radio Estimation Using a Semi-Automatically Created Indoor Building Information**  
  *2024 WCNC* · [Paper](https://ieeexplore.ieee.org/document/10571287) · <sub>CCF C</sub>
- **Deep Machine Learning-Based AoD Map and AoA Map Construction for Wireless Networks**  
  *2024 VTC2024-Spring* · [Paper](https://ieeexplore.ieee.org/document/10683141) · <sub>CCF None</sub>
- **Deep Transfer Learning Based Radio Map Estimation for Indoor Wireless Communications**  
  *IEEE SPAWC 2022* · [Paper](https://ieeexplore.ieee.org/document/9833974) · <sub>CCF None</sub>
- **Distributed Radio Map Reconstruction Based on Semi-Asynchronous Federated Learning Generative Adversarial Networks**  
  *2024 ICCC Workshops* · [Paper](https://ieeexplore.ieee.org/document/10693836) · <sub>CCF None</sub>
- **Evaluation of Transformer Empowered Channel Prediction for 5G and Beyond Communication**  
  *2024 VTC2024-Fall* · [Paper](https://ieeexplore.ieee.org/document/10757983) · <sub>CCF None</sub>
- **Exemplar-Based Radio Map Reconstruction of Missing Areas Using Propagation Priority**  
  *2022 IEEE Global Communications Conference* · [Paper](https://ieeexplore.ieee.org/document/10001269) · <sub>CCF C</sub>
- **Extending Machine Learning Based RF Coverage Predictions to 3D**  
  *2022 AP-S/URSI* · [Paper](https://ieeexplore.ieee.org/document/9887000) · <sub>CCF None</sub>
- **Fast and Accurate Cooperative Radio Map Estimation Enabled by GAN**  
  *IEEE ICC wksp 2024* · [Paper](https://ieeexplore.ieee.org/document/10615679) · <sub>CCF None</sub>
- **Fast Indoor Radio Propagation Prediction using Deep Learning**  
  *2024 EuCAP* · [Paper](https://ieeexplore.ieee.org/document/10500989) · <sub>CCF None</sub>
- **Federated Learning-Based Radio Environment Map Construction for Wireless Networks**  
  *2023 IEEE Global Communications Conference* · [Paper](https://ieeexplore.ieee.org/document/10437765) · <sub>CCF C</sub>
- **FedRME: Federated Learning for Enhanced Distributed Radiomap Estimation**  
  *IEEE VTC-Fall 2024* · [Paper](https://ieeexplore.ieee.org/document/10757742) · <sub>CCF None</sub>
- **Fine Tuning an AI-Based Indoor Radio Propagation Model with Crowd-Sourced Data**  
  *2024 EuCAP* · [Paper](https://ieeexplore.ieee.org/document/10500905) · <sub>CCF None</sub>
- **IndoorRSSINet - Deep learning based 2D RSSI map prediction for indoor environments with application to wireless localization**  
  *IEEE COMSNETS 2023* · [Paper](https://ieeexplore.ieee.org/document/10041394) · <sub>CCF None</sub>
- **IRGAN: cGAN-based Indoor Radio Map Prediction**  
  *IEEE IFIP Networking 2023* · [Paper](https://ieeexplore.ieee.org/document/10186359) · <sub>CCF None</sub>
- **Learning Graph Convolutional Neural Networks to Predict Radio Environment Maps**  
  *2022 WPMC* · [Paper](https://ieeexplore.ieee.org/document/10014842) · <sub>CCF None</sub>
- **Locswinunet: A Neural Network for Urban Wireless Localization Using TOA and RSS Radio Maps**  
  *2023 MLSP* · [Paper](https://ieeexplore.ieee.org/document/10285974) · <sub>CCF None</sub>
- **LocUNet: Fast Urban Positioning Using Radio Maps and Deep Learning**  
  *2022 ICASSP* · [Paper](https://ieeexplore.ieee.org/document/9747240) · <sub>CCF B</sub>
- **Machine Learning-based Predictive Channel Modeling for 6G Wireless Communications Using Image Semantic Segmentation**  
  *IEEE PIMRC 2024* · [Paper](https://ieeexplore.ieee.org/document/10294072) · <sub>CCF None</sub>
- **Optimal Base Station Sleep Control via Multi-Agent Reinforcement Learning with Data-Driven Radio Environment Map Calibration**  
  *2024 VTC2024-Spring* · [Paper](https://ieeexplore.ieee.org/document/10683549) · <sub>CCF None</sub>
- **Prediction of Indoor Wireless Coverage from 3D Floor Plans Using Deep Convolutional Neural Networks**  
  *IEEE LCN 2021* · [Paper](https://ieeexplore.ieee.org/document/9525019) · <sub>CCF C</sub>
- **Propagation Graph Representation Learning and Its Implementation in Direct Path Representation**  
  *2023 WCNC* · [Paper](https://ieeexplore.ieee.org/document/10118812) · <sub>CCF C</sub>
- **Radio DIP - Completing Radio Maps using Deep Image Prior**  
  *IEEE GlobeCom 2023* · [Paper](https://ieeexplore.ieee.org/document/10437485) · <sub>CCF C</sub>
- **Radio Map Estimation (RME) with Deep Progressive Network**  
  *2024 MIPR* · [Paper](https://ieeexplore.ieee.org/document/10708042) · <sub>CCF None</sub>
- **Radio Map Estimation Using a Generative Adversarial Network and Related Business Aspects**  
  *IEEE WPMC 2021* · [Paper](https://ieeexplore.ieee.org/document/9700474) · <sub>CCF None</sub>
- **Radio Map Estimation with Deep Dual Path Autoencoders and Skip Connection Learning**  
  *IEEE PIMRC 2024* · [Paper](https://ieeexplore.ieee.org/document/10293748) · <sub>CCF None</sub>
- **Radio Map Reconstruction Based on Transformer from Sparse Measurement**  
  *2024 ICCT* · [Paper](https://ieeexplore.ieee.org/document/10946503) · <sub>CCF None</sub>
- **RadioResUNet: Wireless Measurement by Deep Learning for Indoor Environments**  
  *2022 WPMC* · [Paper](https://ieeexplore.ieee.org/document/10014763) · <sub>CCF None</sub>
- **RecuGAN: A Novel Generative AI Approach for Synthesizing RF Coverage Maps**  
  *2024 ICCCN* · [Paper](https://ieeexplore.ieee.org/document/10637562) · <sub>CCF C</sub>
- **RM-Gen: Conditional Diffusion Model-Based Radio Map Generation for Wireless Networks**  
  *IEEE IFIP Networking 2024* · [Paper](https://ieeexplore.ieee.org/document/10619829) · <sub>CCF None</sub>
- **RobUNet: A Radio Map Construction Method with A Strong Generalization Capability**  
  *2024 IEEE Global Communications Conference* · [Paper](https://ieeexplore.ieee.org/document/10901772) · <sub>CCF C</sub>
- **Spatial Prediction of Channel Signal Strength Map Using Deep Fully Convolutional Neural Network**  
  *2022 56th Asilomar Conference on Signals, Systems, and Computers* · [Paper](https://ieeexplore.ieee.org/document/10051926) · <sub>CCF None</sub>
- **Three-Dimensional Radio Spectrum Map Prediction Based on Fully Connected Neural Network**  
  *2023 ICAIT* · [Paper](https://ieeexplore.ieee.org/document/10367287) · <sub>CCF None</sub>
- **Towards the Metaverse: Distributed Radio Map Reconstruction based on Federated Learning Generative Adversarial Networks**  
  *2024 IWCMC* · [Paper](https://ieeexplore.ieee.org/document/10592435) · <sub>CCF None</sub>
- **Transformer based Radio Map Prediction Model for Dense Urban Environments**  
  *IEEE ISAPE 2021* · [Paper](https://ieeexplore.ieee.org/document/9753644) · <sub>CCF None</sub>
- **Transformer-Based Neural Surrogate for Link-Level Path Loss Prediction from Variable-Sized Maps**  
  *IEEE GlobeCom 2023* · [Paper](https://ieeexplore.ieee.org/document/10436935) · <sub>CCF C</sub>
- **UAV-aided Joint Radio Map and 3D Environment Reconstruction using Deep Learning Approaches**  
  *IEEE ICC 2022* · [Paper](https://ieeexplore.ieee.org/document/9838964) · <sub>CCF C</sub>
- **UnetRay: A Prediction Method of Indoor Radio Signal Strength Distribution**  
  *IEEE ICAIT 2023* · [Paper](https://ieeexplore.ieee.org/document/10367372) · <sub>CCF None</sub>

</details>

### Preprints

<details open>
<summary><strong>2026</strong> <sub>28 papers</sub></summary>

- **A Geometric Algebra-Informed 3DGS Framework for Wireless Channel Prediction**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2605.19065)
- **Active Perception for Radio Map Reconstruction in Uncharted 3D Air-Ground Environments**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2606.12844)
- **BDFlow-3DRM: Height-Coherent 3D Radio Map Construction via Bi-Dynamical Flow Matching**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2607.09778)
- **Beam-Aware Radio Map Estimation With Physics-Consistent Parametric Modeling for Unknown Multiple Satellites**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2605.07763)
- **Channel Knowledge Map Enabled Low-Complexity Dynamic Radio Environment Reconstruction**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2608.17322)
- **CKM Beyond Channel Gain: Spatial Correlation Map Construction with Deep Learning**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2604.20684)
- **Construction and Dynamic Update of Channel Gain Maps via 3D Gaussian Splatting**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2607.21099)
- **CORF-GS: Real-Time Wireless Radiance Field Reconstruction via Coupled Optical-RF Gaussian Splatting**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2607.25569)
- **Dynamic Channel Knowledge Map: Fundamentals, Construction, and Applications**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2607.17133)
- **EA-RMENet: Path Loss Prediction in Urban Environments Using Deep Learning**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2607.16449)
- **Eff-WRFGS: Efficient Wireless Radiance Field Using 3D Gaussian Splatting**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2605.15324)
- **FARM: Foundational Aerial Radio Map for Intelligent Low-Altitude Networking**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2604.17362)
- **GSBF: Gaussian Splatting for Environment-Aware Beamforming**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2608.05896)
- **OctCGS: Octree-Contextual Gaussian Splatting with Explicit Multi-Order Propagation Modeling for Channel Knowledge Map Construction**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2605.22961)
- **PALM: Single-Station Super-Resolved Small-Scale Radio-Map Localization by Path-Atom Matching**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2606.16157)
- **Physics-Aware Query-Conditioned Graph Attention Networks for Radio Map Estimation**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2604.17414)
- **Physics-Unrolled Neural Operator for Wireless Field Modeling**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2608.18495)
- **QuaMoE-DRF: Proactive Beam and Rate Adaptation via Multimodal Dynamic Radio Map Forecasting in ISAC Networks**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2607.00974)
- **Radio Map Updating from Streaming Spectrum Measurements via Memory-Based Online Gaussian Processes**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2607.17954)
- **Radio-Frequency Inverse Rendering for Wireless Environment Modeling**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2604.07086)
- **RMPrior: Bridging Propagation Priors and Diffusion Refinement for Efficient Radio Map Construction**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2606.03074)
- **RxGS: Receiver-Generalizable 3D Gaussian Splatting for Radio-Frequency Data Synthesis**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2605.24290)
- **Sparse Gain Radio Map Reconstruction With Geometry Priors and Uncertainty-Guided Measurement Selection**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2604.05788)
- **Statistical Channel Fingerprint Construction for Massive MIMO: A Unified Tensor Learning Framework**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2604.27574)
- **TGPP: Trajectory-Guided Plug-and-Play Priors for Sparse Radio Map Reconstruction**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2605.05844)
- **Theoretical Analysis of Diffusion Models for Radio Map Estimation with Ultra-Low Sampling Rates**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2606.25310)
- **Transmitter-Agnostic Radio Map Reconstruction via Adversarial Cooperative Inference**  
  *SSRN 2026* · [Paper](https://doi.org/10.2139/ssrn.7304218)
- **Voxel-CKM: Voxelized Radio Frequency Radiance Fields for Fast and Few-Shot CKM Construction**  
  *arXiv 2026* · [Paper](https://arxiv.org/abs/2606.03531)

</details>

<details open>
<summary><strong>2025</strong> <sub>15 papers</sub></summary>

- **BS-1-to-N: Diffusion-Based Environment-Aware Cross-BS Channel Knowledge Map Generation for Cell-Free Networks**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2507.23236)
- **Channel Fingerprint Construction for Massive MIMO: A Deep Conditional Generative Approach**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2505.07893)
- **Diffusion2: Turning 3D Environments into Radio Frequency Heatmaps**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2510.02274)
- **ExposNet: A Deep Learning Framework for EMF Exposure Prediction in Complex Urban Environments**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2503.02966)
- **Fusion of Pervasive RF Data with Spatial Images via Vision Transformers for Enhanced Mapping in Smart Cities**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2508.03736)
- **GenMetaLoc: Learning to Learn Environment-Aware Fingerprint Generation for Sample Efficient Wireless Localization**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2503.18078)
- **GSpaRC: Gaussian Splatting for Real-Time Reconstruction of RF Channels**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2511.22793)
- **LLM4MG: Adapting Large Language Model for Multipath Generation via Synesthesia of Machines**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2509.14711)
- **Machine Learning based Radio Environment Map Estimation for Indoor Visible Light Communication**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2507.19149)
- **Mip-NeWRF: Enhanced Wireless Radiance Field with Hybrid Encoding for Channel Prediction**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2511.09150)
- **PINN and GNN-based RF Map Construction for Wireless Communication Systems**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2507.22513)
- **RadioDiff-Loc: Diffusion Model Enhanced Scattering Congnition for NLoS Localization with Sparse Radio Map Estimation**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2509.01875)
- **Temporal Spectrum Cartography in Low-Altitude Economy Networks: A Generative AI Framework with Multi-Agent Learning**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2505.15571)
- **UAV-enabled Adaptive Radio Sampling for High-quality Radio Environment Map Construction in Unknown Environments**  
  *TechRxiv 2025* · [Paper](https://doi.org/10.36227/techrxiv.175993459.99080577/v1)
- **Wideband RF Radiance Field Modeling Using Frequency-Embedded 3D Gaussian Splatting**  
  *arXiv 2025* · [Paper](https://arxiv.org/abs/2505.20714)

</details>

<details open>
<summary><strong>2024 and Earlier</strong> <sub>6 papers</sub></summary>

- **A Deep Unfolding-Based Scalarization Approach for Power Control in D2D Networks**  
  *arXiv 2024* · [Paper](https://arxiv.org/abs/2410.16104)
- **Deep Learning Based Active Spatial Channel Gain Prediction Using a Swarm of Unmanned Aerial Vehicles**  
  *arXiv 2023* · [Paper](https://arxiv.org/abs/2310.04547)
- **RadioNet: Transformer based Radio Map Prediction Model For Dense Urban Environments**  
  *arXiv 2021* · [Paper](https://arxiv.org/abs/2105.07158)
- **Simulating, Fast and Slow: Learning Policies for Black-Box Optimization**  
  *arXiv 2024* · [Paper](https://arxiv.org/abs/2406.04261)
- **Solving Maxwell's equations with Non-Trainable Graph Neural Network Message Passing**  
  *arXiv 2024* · [Paper](https://arxiv.org/abs/2405.00814)
- **White Paper on Radio Channel Modeling and Prediction to Support Future Environment-aware Wireless Communication Systems**  
  *arXiv 2023* · [Paper](https://arxiv.org/abs/2309.17088)

</details>

---

## Contributing

Contributions are welcome. Please provide the paper title, publication venue and year, paper URL, and an in-paper code citation when requesting placement in **Papers with Code**.
